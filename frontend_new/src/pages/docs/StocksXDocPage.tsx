...
export function StocksXDocPage() {
  const { t, ready, loadError, lang } = useDocBundle('stocksx')

  if (!ready) {
    return <DocBundleLoadingShell variant="stocks" loadError={loadError} />
  }

  return (
    <DocLayout variant="stocks">
      <DocNavbar t={t} lang={lang} moduleNav={{ mode: 'docs', current: 'stocksx' }} />
      <main className="doc-main">
        <div className="doc-main-inner">
          <section className="doc-hero" aria-labelledby="doc-hero-title">
            <div className="doc-hero-card">
              ...
              <div className="doc-hero-body">
                <span className="doc-hero-badge">QUANT TRADING</span>
                <h1 id="doc-hero-title">StocksX</h1>
          ...
          <article className="doc">
            <DocReadingSummary t={t} variant="stocksx" />
            <h2>{t('h_feat')}</h2>
            <ul>
              <li>{t('f1')}</li>
              <li>{t('f2')}</li>
            ...
            <p>
              <Link className="doc-lab-cta" to={{ pathname: PATHS.labs.stocksx, search: toLangSearch(lang) }}>
                開啟大螢幕儀表板（示範資料）→
              </Link>
            </p>
          </article>
