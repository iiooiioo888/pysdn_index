...
export function SuperNovaDocPage() {
  const { t, ready, loadError, lang } = useDocBundle('supernova')

  if (!ready) {
    return <DocBundleLoadingShell variant="nova" loadError={loadError} />
  }

  return (
    <DocLayout variant="nova">
      <DocNavbar t={t} lang={lang} moduleNav={{ mode: 'docs', current: 'supernova' }} />
      <main className="doc-main">
        <div className="doc-main-inner">
          <section className="doc-hero" aria-labelledby="doc-hero-title">
            <div className="doc-hero-card">
              ...
              <div className="doc-hero-body">
                <span className="doc-hero-badge">DATA COLLECTION</span>
                <h1 id="doc-hero-title">SuperNova</h1>
          ...
          <article className="doc">
            <DocReadingSummary t={t} variant="supernova" />
            <h2>{t('h_feat')}</h2>
            <ul>
              <li>{t('f1')}</li>
              <li>{t('f2')}</li>
            ...
            <h2>{t('h_platforms')}</h2>
            <div dangerouslySetInnerHTML={{ __html: sanitizeHtml(t('platforms_html')) }} />
            ...
            <p>
              <Link className="doc-lab-cta" to={{ pathname: PATHS.labs.supernova, search: toLangSearch(lang) }}>
                開啟大螢幕儀表板（示範資料）→
              </Link>
            </p>
          </article>
